...
void main() {
    ...
    // wire [31:0] gpio_in = ((second_toggle &1'b1) << 0)|((UP_DWN & 1'b1)<<1);
      ...
      // Read UP_DOWN 
      if(reg_gpio&0b10) //connect wire to pause; disconnect to resume count
      {
          second_timer++;
          //incrementing = true;
      }
      ...
      // Read RESET
      if(!(reg_gpio&0b100)) //connect wire to reset to 0; disconnect to resume count
      {
        laps = 0;
        second_timer = 0;
      }
      ...
      // Read LAP
      if(!(reg_gpio&0b1000)) //connect wire to increment lap count; disconnect does nothing
      {
         if (!(lap_counted))
        {
            laps++;
    ...
    // Following code works when "/" and "%" implemented
       ...
       reg_gpio = (laps << 12) | (min_ones << 8) | (sec_tens<<4) | sec_ones;
